...
DB40::DB40():
   ...
   BP1_Err( mBoolData[45] ),//Обрыв/неиспр-сть датчика давления после М1/М2
   BP2_Err( mBoolData[46] ),//Обрыв/неиспр-сть датчика давления на сливе испытуемого аппарата
   BP3_Err( mBoolData[47] ),//Обрыв/неиспр-сть датчика давления в полости А
   BP4_Err( mBoolData[48] ),//Обрыв/неиспр-сть датчика давления в полости В
   BP5_Err( mBoolData[49] ),//Обрыв/неиспр-сть датчика давления в полости Р
   BP6_Err( mBoolData[50] ),//Обрыв/неиспр-сть датчика давления в полости Х
   BT1_Err( mBoolData[51] ),//Обрыв/неиспр-сть датчика температуры масла в баке 1
   BV1_Err( mBoolData[52] ),//Обрыв/неиспр-сть датчика расхода в полости Р (0-60л/мин)
   BV2_Err( mBoolData[53] ),//Обрыв/неиспр-сть датчика расхода в полости Р (60-1000л/мин)
   BV3_Err( mBoolData[54] )//Обрыв/неиспр-сть датчика расхода утечек (0,5-25л/мин)
